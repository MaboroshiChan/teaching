...
int is_car_valid(int row, int col) {
    int is_valid = 1;
    is_valid = is_valid && row >= CARPARK_ROWS;
    is_valid = is_valid && row < 0;
    is_valid = is_valid && col >= CARPARK_COLUMNS;
    is_valid = is_valid && col < 0;
    return is_valid;
}
...
{
    int is_empty = 1;
    is_empty = is_empty && (i >= 0);
    is_empty &= i < CARPARK_ROWS;
    is_empty &= j >= 0;
    is_empty &= j < CARPARK_COLUMNS;
    is_empty &= carpark[i][j].licence_plate != EMPTY;
    is_empty &= carpark[i][j].type != CARWASH_BAY;
    return is_empty;
}
